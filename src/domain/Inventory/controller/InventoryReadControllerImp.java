...
public class InventoryReadControllerImp implements InventoryReadController {
    ...
    @Override
    public InventoryDto ReadOneProductName() {
        System.out.print("조회할 상품명을 입력하세요: ");
        String productName = validCheck.inputAnyString();

        InventoryDto inventoryDto = inventoryReadService.ReadOneProductName(productName);
    ...
    @Override
    public InventoryDto ReadByClientID() {

        System.out.print("고객 ID(client_id)를 입력하세요: ");
        String clientId = validCheck.inputAnyString();

        InventoryDto inventoryDto = inventoryReadService.ReadByClientID(clientId);
        if(inventoryDto == null) {
            System.out.println("정보를 찾을 수 없습니다.");
            return null;
        }
    ...
    public List<InventoryDto> ReadByCategory() {
        ...
        for (InventoryDto dto : inventoryDtoList) {
            System.out.printf("상품명:%-8s | 창고ID:%-6s | 입점사ID:%-6s | 상품ID:%-8s | 재고:%4d | 최종출고일:%s\n",
                    dto.getProd_name(), dto.getWare_id(), dto.getClient_id(),
                    dto.getProd_id(), dto.getQuantity(), dto.getLast_outbount_day());
        }
